...
export class BLRaycaster {
    ...
    /**@type {(intersects:import("three").Intersection[])=>import("three").Intersection[]} */
    _filter;
    ...
    /**判断是否对相同的对象重复监听相同事件 */
    exist(object, type) {
        return !!(this.callbackMap.has(object) && this.callbackMap.get(object)[type]);
    }
    ...
    getIntersects = (event, object, type) => {
        ...
        // 自定义过滤结果。
        const result = (this._filter || this._defaultFilterFn)(this.intersects);

        this.callbackMap.get(object)[type].forEach(cb => cb(result, event));
    };

    /**
     * 对射线检测的结果进行过滤
     * @param {(intersects:import("three").Intersection[])=>import("three").Intersection[]} filterFn 过滤函数
     */
    setFilter(filterFn) {
    ...
    _defaultFilterFn(intersects) {
        // 过滤不可见对象
        return intersects.filter(e => e.object.visible);
    }
}
