import { Vector3,Vector2,Camera,Sphere,Box3 } from "three";
 ...
 * @param { "x"|"y"|"z" } param1 保留参数1
 * @param { "x"|"y"|"z" } param2 保留参数2
 * @returns { Vector2|Vector2[] }
 */
function vec3ToVec2(points,param1 = "x",param2 = "y") {
    if (Array.isArray(points)) {
        const results = [];
        points.forEach(point => results.push(vec3ToVec2(point,param1,param2)));
        return results;
    } else {
        return new Vector2(points[param1],points[param2]);
    }
}
 ...
 * @returns
 */
function getBoxAndSphere(object) {
